...
from .models import CourseClassroom, Course
...
def update_course_capacity_on_save(sender, instance, **kwargs):
    ...
    total_capacity = course.classrooms.aggregate(total=Sum('capacity'))['total']
...
def update_course_capacity_on_delete(sender, instance, **kwargs):
    ...
    total_capacity = course.classrooms.exclude(id=instance.classroom.id).aggregate(total=Sum('capacity'))['total']
    course.capacity = total_capacity
    course.save()
...
def update_remaining_capacity_on_enrollment_change(sender, instance, **kwargs):
    ...
    if instance.status == 'enrolled':
        ...
        instance.course.remaining_capacity = instance.course.capacity - instance.course.enrollment_set.filter(status='enrolled').count()
